...
export default function Navbar() {
  ...
  const { getTotalCompleted, user, logout } = useApp();
    ...
    <nav className={styles.navbar}>
      <div className={styles.inner}>
        ...
        <div className={styles.meta}>
          ...
          {user ? (
            <button onClick={logout} className="btn btn-secondary btn--sm" style={{ marginLeft: "8px" }}>
              Log out
            </button>
          ) : (
            <Link href="/login" className="btn btn-primary btn--sm" style={{ marginLeft: "8px" }}>
              Log in
            </Link>
          )}
        </div>
